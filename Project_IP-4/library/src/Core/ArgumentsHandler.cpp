...
bool ArgumentsHandler::valueIsValid(string value) const
{
    ...
    if ((option == "--sndft" || option == "--fndft" || option == "--sidft" || option == "--fidft") && stoi(value) > 1)
        return false;
    if (option == "--lpfilter" && stoi(value) > 100) return false;
    return true;
}
...
TASK 2:
    ...
    --lpfilter <0, 100>
        Performs an ideal low pass filter, the value is the cut-off frequency calculated as the percentage of the maximum frequency.
)";
}
